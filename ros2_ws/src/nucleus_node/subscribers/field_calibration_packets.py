...
class SubscriberFieldCalibrationPackets(Node):
    ...
    def __init__(self, callback_function, topic='field_calibration', qos_profile=100):

        super().__init__('subscriber_field_calibration_packets')

        self.subscription = self.create_subscription(FieldCalibration, topic=topic, callback=callback_function, qos_profile=qos_profile)
