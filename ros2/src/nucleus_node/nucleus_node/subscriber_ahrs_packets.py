...
class SubscriberAhrsPackets(Node):
    ...
    def __init__(self, callback_function):
        ...
        #self.subscription = self.create_subscription(Ahrs, 'ahrs', self.ahrs_packet_callback, 100)
...
def main(args=None):
    ...
    def ahrs_packet_callback(ahrs):
        #self.get_logger().info(f'NEW # roll: {ahrs.roll} \t pitch: {ahrs.pitch} \t heading: {ahrs.heading}')
        subscriber.get_logger().info(f'NEW # roll: {ahrs.roll} \t pitch: {ahrs.pitch} \t heading: {ahrs.heading}')
        #print(f'NEW # roll: {ahrs.roll} \t pitch: {ahrs.pitch} \t heading: {ahrs.heading}')
